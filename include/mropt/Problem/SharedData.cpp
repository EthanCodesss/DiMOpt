...
void SharedData::initializeZandMu(const mropt::collisions::Collisions &col,
                                  int proc_id) {
  // Compute once
  if (proc_id == 0) {
...
void SharedData::init_sync(std::shared_ptr<DistributedRobot> &robot) {
  ...
  MPI_Allgather(robot->shape->get_safety_radius_ptr(), 1, MPI_DOUBLE,
                robot->data_shared->safe_distance, 1, MPI_DOUBLE,
                MPI_COMM_WORLD);
  // Share dynamic violations
  double dynamic_viol = robot->dynamics_violation.scalar();
  MPI_Allgather(&dynamic_viol, 1, MPI_DOUBLE,
                robot->data_shared->dynamic_violation, 1, MPI_DOUBLE,
                MPI_COMM_WORLD);
  // Share XY initial guess for all robots
  MPI_Allgather(
      robot->X_curr((int)mropt::StateSpace::State::POS::x, all)->data(), N + 1,
      MPI_DOUBLE, robot->data_shared->x, N + 1, MPI_DOUBLE, MPI_COMM_WORLD);
  MPI_Allgather(
      robot->X_curr((int)mropt::StateSpace::State::POS::y, all)->data(), N + 1,
      MPI_DOUBLE, robot->data_shared->y, N + 1, MPI_DOUBLE, MPI_COMM_WORLD);
  // Initialize z and mu with initial guess values
  robot->data_shared->initializeZandMu(robot->collisions_d->col_,
                                       robot->robot_id);
}

// 函数的主要目的是同步机器人的当前状态x_curr和y_curr到共享数据中
void SharedData::sync(std::shared_ptr<DistributedRobot> &robot) {
  ...
  // Gather new solution
  // MPI_Allgather 是一个MPI函数，用于在所有进程中收集和分发数据
  MPI_Allgather(
      robot->X_curr((int)mropt::StateSpace::State::POS::x, all)->data(), N + 1,
      MPI_DOUBLE, robot->data_shared->x, N + 1, MPI_DOUBLE, MPI_COMM_WORLD);
  MPI_Allgather(
...
void SharedData::update_z_mu12(const mropt::collisions::Collisions &col,
                               int proc_id) {
  // Compute once
  ...
  if (proc_id == 0) {
    for (int r1 = 0; r1 < R; ++r1) {
      for (int k = 0; k < N + 1; ++k) {
        auto z_x_new =
            Xcurr(r1, k)((int)mropt::StateSpace::State::POS::x).scalar();
        /* auto z_x_prev = Xprev(r1,k)((int)
         * mropt::StateSpace::State::POS::x).scalar();*/
        auto z_y_new =
            Xcurr(r1, k)((int)mropt::StateSpace::State::POS::y).scalar();
        /* auto z_y_prev = Xprev(r1,k)((int)
         * mropt::StateSpace::State::POS::y).scalar();*/

        auto z_x_prev =
            Z_val(r1, k)((int)mropt::StateSpace::State::POS::x).scalar();
        auto z_y_prev =
            Z_val(r1, k)((int)mropt::StateSpace::State::POS::y).scalar();
        auto z_x_prev_prev =
            Z_val_p(r1, k)((int)mropt::StateSpace::State::POS::x).scalar();
        auto z_y_prev_prev =
            Z_val_p(r1, k)((int)mropt::StateSpace::State::POS::y).scalar();
        ...
        double b = (R - 1) / R;
        auto real_z_x =
            ((double)1 / W) * z_x_new + ((double)(W - 1) / W) * z_x_prev;
        auto real_z_y =
            ((double)1 / W) * z_y_new + ((double)(W - 1) / W) * z_y_prev;
        /*setZval(r1,k, real_z_x + b*(real_z_x- z_x_prev) +
           (b*0.5)*(z_x_prev-z_x_prev_prev),
                real_z_y + b*(real_z_y-z_y_prev) +
           (b*0.5)*(z_y_prev-z_y_prev_prev));*/
        setZval(r1, k, real_z_x + b * (z_x_new - z_x_prev),
                real_z_y + b * (z_y_new - z_y_prev));
        /*setZval(r1,k, real_z_x + b*(z_x_new- z_x_prev) + (1-b)*(z_x_prev
           -z_x_prev_prev), real_z_y + b*(z_y_new-z_y_prev) + (1-b)*(z_y_prev
           -z_y_prev_prev));*/
        ...
        auto new_mu12 = Mu12_val(r1, k) + (Xcurr(r1, k) - Z_val(r1, k));
        /*auto new_mu12 = Mu12_val(r1, k) +
                   (Z_val(r1, k) - Z_prev(r1, k));*/
        auto mu12_x_new =
            new_mu12((int)mropt::StateSpace::State::POS::x).scalar();
        auto mu12_y_new =
            new_mu12((int)mropt::StateSpace::State::POS::y).scalar();
        auto prev_mu12 = Mu12_val(r1, k);
        auto mu12_x_prev =
            prev_mu12((int)mropt::StateSpace::State::POS::x).scalar();
        auto mu12_y_prev =
            prev_mu12((int)mropt::StateSpace::State::POS::y).scalar();
        /*setMu12val(r1,k,
        ...
        /* setZval(r1, k, z_x_new + (1/(*rho))*mu12_x_prev, z_y_new
         * +(1/(*rho))*mu12_y_prev);*/
        // setZval(r1, k, z_x_new , z_y_new );
        // setZval(r1, k, 0.5*z_x_new + 0.5*z_x_prev, 0.5*z_y_new +
        // 0.5*z_y_prev);
      }
    }
  }
...
/*void SharedData::update_z_mu12(const mropt::collisions::Collisions & col, int
  proc_id){
  //Compute once
  if(proc_id == 0) {
    for (int r1 = 0; r1 < R; ++r1) {
      for (int r2 = 0; r2 < R; ++r2) {
        for (int k = 0; k < N + 1; ++k) {
          ...
          const auto &total_safe_dist
              = getSafetyDistance(r1) + getSafetyDistance(r2);
          const auto &g12
              = col.f(Xcurr(r1, k), Xprev(r2, k)).scalar() + total_safe_dist *
  total_safe_dist; const auto &g21 = col.f(Xcurr(r2, k), Xprev(r1, k)).scalar()
  + total_safe_dist * total_safe_dist;
          */
...
double SharedData::dynamicsViolation() {
  ...
  for (int r = 0; r < R; r++) {
    max_violation = std::max(max_violation, this->dynamic_violation[r]);
  }
  ...
  for (int r1 = 0; r1 < R; ++r1) {
    for (int r2 = 0; r2 < R; ++r2) {
      for (int k = 0; k < N + 1; ++k) {
        ...
        const auto &total_safe_dist =
            getSafetyDistance(r1) + getSafetyDistance(r2);
        const auto &g_real = col.f(Xcurr(r1, k), Xcurr(r2, k)).scalar() +
                             total_safe_dist * total_safe_dist;
...
double SharedData::actual_cost(std::shared_ptr<DistributedRobot> &robot) {
  ...
  double local_cost =
      robot->cost->J(robot->X_curr, robot->U_curr, t0f).scalar();
  double global_sum;
...
void SharedData::plot(std::shared_ptr<DistributedRobot> &robot,
                      double solve_time_r) {
  if (robot->robot_id == 0) {
    double g12_{0.0};
    double g21_{0.0};
    ...
    for (int r1 = 0; r1 < R; ++r1) {
      for (int r2 = r1 + 1; r2 < R; ++r2) {
        for (int k = 0; k < N + 1; ++k) {
          ...
          g12_ =
              g12_ +
              robot->collisions_d->col_.f(Xcurr(r1, k), Xprev(r2, k)).scalar() +
              total_safe_dist * total_safe_dist;
          g21_ =
              g21_ +
              robot->collisions_d->col_.f(Xcurr(r2, k), Xprev(r1, k)).scalar() +
              total_safe_dist * total_safe_dist;
          greal_ =
              greal_ +
              robot->collisions_d->col_.f(Xcurr(r1, k), Xcurr(r2, k)).scalar() +
              total_safe_dist * total_safe_dist;
          // z_ = z_ + Z_val(r1, r2, k);

          auto dist =
              std::sqrt(-robot->collisions_d->col_.f(Xcurr(r1, k), Xcurr(r2, k))
                             .scalar()) -
              total_safe_dist;
          if (dist < min_clear)
            min_clear = dist;
  ...
  if (robot->robot_id == 0) {
    ...
    for (int r = 0; r < R; ++r) {
      ...
      std::for_each(traj_r_percent.begin(), traj_r_percent.end(),
                    [&](double &num) { num = num / traj_r[r][0]; });
...
/*void SharedData::plot(std::shared_ptr<DistributedRobot> & robot, double
solve_time_r) { if(robot->robot_id == 0) { double g12_{0.0}; double g21_{0.0};
    double z_{0.0};
    double greal_{0.0};
    double min_clear{INT_MAX};
    for (int r1 = 0; r1 < R; ++r1) {
      for (int r2 = r1 + 1; r2 < R; ++r2) {
        for (int k = 0; k < N + 1; ++k) {
          ...
          const auto &total_safe_dist
              = getSafetyDistance(r1) + getSafetyDistance(r2);
          g12_ = g12_ + robot->collisions_d->col_.f(Xcurr(r1, k), Xprev(r2,
k)).scalar() + total_safe_dist * total_safe_dist; g21_ = g21_ +
robot->collisions_d->col_.f(Xcurr(r2, k), Xprev(r1, k)).scalar() +
              total_safe_dist * total_safe_dist;
          greal_ = greal_ + robot->collisions_d->col_.f(Xcurr(r1, k), Xcurr(r2,
k)).scalar() + total_safe_dist * total_safe_dist; z_ = z_ + Z_val(r1, r2, k);

          auto dist = std::sqrt(-robot->collisions_d->col_.f(Xcurr(r1, k),
Xcurr(r2, k)).scalar() ) - total_safe_dist; if(dist < min_clear) min_clear =
dist;
        }
      }
    }
  ...
  // Cost of the multirobot system
  DM t0f = DM({robot->p_.t0, robot->p_.tf});
  double local_cost = robot->cost->J(robot->X_curr, robot->U_curr,
t0f).scalar(); auto *global_sum = new double[R]; MPI_Gather(&local_cost,1,
MPI_DOUBLE, global_sum,1,  MPI_DOUBLE, 0, MPI_COMM_WORLD);
...
void SharedData::getMRTrajectory(
  ...
  if (robot->robot_id == 0) {
    ...
    for (int r = 0; r < R; ++r) {
      x.push_back(
          Xcurr(r)((int)mropt::StateSpace::State::POS::x, all).get_elements());
      y.push_back(
          Xcurr(r)((int)mropt::StateSpace::State::POS::y, all).get_elements());
    }
  }
  // Gather theta
  ...
  MPI_Allgather(
      robot->X_curr((int)mropt::StateSpace::State::POS::o, all)->data(), N + 1,
      MPI_DOUBLE, o_mr, N + 1, MPI_DOUBLE, MPI_COMM_WORLD);
  ...
  // Controls
  u = std::vector<std::vector<std::vector<double>>>(
      R, std::vector<std::vector<double>>(
             robot->cs->nu(), std::vector<double>(robot->cs->Nu())));
  for (int u_id = 0; u_id < robot->cs->nu(); ++u_id) {

    auto *u_mr_i = new double[R * (robot->cs->Nu())];
    MPI_Gather(robot->u_sol[u_id].data(), robot->cs->Nu(), MPI_DOUBLE, u_mr_i,
               robot->cs->Nu(), MPI_DOUBLE, 0, MPI_COMM_WORLD);
