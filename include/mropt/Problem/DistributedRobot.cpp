...
DistributedRobot::DistributedRobot(
    int robot_id_, const Robot::Params &P,
    ...
    : Robot(P, Cs, Ss, Cost, Fspace, Dynamics, Shape), collisions_d(Collisions),
      robot_id(robot_id_) {}
  ...
  while (true) {  // Constraints Violation
    ...
    while (true) { // Approximation Quality (convexity)
      ...
      while (true) {             // Trust Region Quality
        ...
        // Compute robot cost true and model, constraint violations and set
        // solution
        prev_true_cost = 0.0;
        true_cost = 0.0;
        model_cost = 0.0;
      ...
      if (failed)
        break; // trust radius region is minimum || backtrack()
      if (delta_f.scalar() <= f_tol || DM::mmax(delta_x).scalar() < x_tol)
        break;
    ...
    if (std::sqrt(collisions_violation.scalar()) >
        this->collisions_d->col_.threshold) {
      mu_c_0 = k * mu_c_0;
    ...
    if (dynamics_violation.scalar() > g_tol &&
        std::sqrt(collisions_violation.scalar()) >
            this->collisions_d->col_.threshold) {
      mu_d_0 = k * mu_d_0; // TODO: x100 for dynamics (testing)
      ocp_->set_value(mu_dynamics, mu_d_0);
      constraint_violation = true;
    ...
    if (!viol_improv(5))
      break;
    if (model_cost.scalar() < 0 || solution->value(J_model_nocol).scalar() < 0)
    ...
    true_cost_ = [&](const DM &X, const DM &U) {
      ...
      // result +=     robot->mu_c_0 *
      // robot->solution->value(robot->col_J_real_);
      result += mu_c_0 * col_J_real_fast_(
                             std::vector<casadi::DM>{{X}, {x2param_list}})[0];
      return result;
    };
  }
...
bool DistributedRobot::backtrack_d() {
  ...
  while (true) {
    ...
    DM dx = X_sol - X_curr;
    DM du = U_sol - U_curr;
    DM delta_true_cost = true_cost_(DM::plus(X_curr, DM::mtimes(t, dx)),
                                    DM::plus(U_curr, DM::mtimes(t, du)));
    DM curr_true_cost = true_cost_(X_curr, U_curr);
    if (delta_true_cost.scalar() <=
        curr_true_cost.scalar() -
            alpha * t * (curr_true_cost.scalar() - model_cost.scalar()))
      break;
    t = t / beta;
    X_curr = X_curr + t * dx;
    U_curr = U_curr + t * du;
...
double DistributedRobot::compute_robot_costs_violations_d() {
  ...
  collisions_violation = col_J_viol_(std::vector<casadi::DM>{
      {X_sol},
      {x2param_list}})[0]; // TODO: test this due to set_J_voil_per_robot
  ...
  if (plot_ || debug_) {
    ...
    std::cout << "True Cost l1: " << this->true_cost_l1penalty(X_sol, U_sol)
              << std::endl;
    std::cout << "True cost Consensus: "
              << collisions_d->J_al_->operator()(
                     std::vector<casadi::DM>{{X_sol}, {*data_shared->rho}})[0]
              << std::endl;
    std::cout << "True cost collisions: "
              << mu_c_0 * col_J_real_fast_(std::vector<casadi::DM>{
                              {X_sol}, {x2param_list}})[0]
              << std::endl;
    std::cout << "Model l1: " << solution->value(J_model_nocol) << std::endl;
    std::cout << "2) Model l1 collisions: "
              << solution->value(mu_col * sum_g_col) << std::endl;
    /* std::cout << "Model Cost col: " << model_cost - model_cost_no_col <<
     std::endl; std::cout << "True Cost No col: " << true_cost -
     true_cost_l1penalty(X_sol,U_sol) << std::endl;*/
    //      std::cout << "Model Cost 2: " << solution->value(ocp_->f()) <<
    //      std::endl;
    std::cout << "Dynamics Violation: " << dynamics_violation << std::endl;
    std::cout << "Dynamics Violation2: "
              << dynamics->J_real(X_sol, U_sol, DM({p_.tf})) << std::endl;
    std::cout << "Dynamics Violation3: " << solution->value(sum_g_dynamics)
              << std::endl;
    std::cout << "Free Space Violation Computed: " << free_space_violation
              << std::endl;
    std::cout << "Collisions Violation : " << collisions_violation << std::endl;
    std::cout << "Collisions Violation Model: " << solution->value(sum_g_col)
              << std::endl;
    std::cout << "Collisions Violation Jreal: " << solution->value(col_J_real_)
              << std::endl;
...
bool DistributedRobot::handle_solution_d(double pho) {
  ...
  if (pho >= pho0) { // accept solution
    delta_x = (sum1(sum2(DM::abs(X_sol - X_curr))) +
               sum1(sum2(DM::abs(U_sol - U_curr)))) /
              (ss->Nx() + cs->Nu());
    X_curr = X_sol;
    U_curr = U_sol;
  }
  ...
  if (pho < 0.0) { // Recovery mode
    ...
    if (backtrack_d()) {
      ...
      delta_x = (sum1(sum2(DM::abs(X_before_bt - X_curr))) +
                 sum1(sum2(DM::abs(U_before_bt - U_curr)))) /
                (ss->Nx() + cs->Nu());
      DM actual_cost_after_bt = cost->J(X_curr, U_curr, t0f);
      // delta_f = delta_f - actual_cost_before_bt + actual_cost_after_bt; //
      // Update overall delta_f (considers fleet)
      delta_f = std::abs(true_cost_(X_before_bt, U_before_bt).scalar() -
                         true_cost_(X_curr, U_curr).scalar()) /
                true_cost_(X_curr, U_curr).scalar(); // TODO: testing this
...
void DistributedRobot::scp_d_nocol() {
  ...
  while (true) { // Constraints Violation
    ...
    while (true) { // Approximation Quality (convexity)
      ...
      if (failed)
        break; // trust radius region is minimum || backtrack()
      if (std::abs(delta_f.scalar()) <= f_tol ||
          DM::mmax(delta_x).scalar() < x_tol) {
        break;
      }
    }
